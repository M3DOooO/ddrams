...
<!-- left menu ends -->
			...
			<a href="reports.php"><span>التقارير</span></a> / <a href="reports_day.php?day=<?php echo $D;?>&month=<?php echo $M;?>&year=<?php echo $Y;?>"><span>تقارير يوم <?php echo $Y?>-<?php echo $M?>-<?php echo $D?></span></a> / <span>تقارير الكاشير</span>
			</div>
			<!-- content starts -->
...
<h2><?php echo $lang_97;?>: <font color='green'><?php echo $money_all;?></font> <?php echo $lang_100;?> </h2> 
...
<tr>
						...
						<tbody>
						<?php 	
 $result2x = mysql_query("SELECT * FROM users GROUP BY Username");
while($row2x = mysql_fetch_array($result2x))
{
 	$cahseer = $row2x['Username'];

$result = mysql_query("SELECT *,SUM(money) FROM `reports` WHERE day = '$D' AND month = '$M'  AND year = '$Y' AND End_hour != '-' AND casheer !='' AND status = 'done'  AND casheer = '$cahseer'");						
while($row = mysql_fetch_array($result))
{
	$money1 = $row['SUM(money)'];
 	 // المصاريف
$result2 = mysql_query("SELECT *,SUM(price) FROM `reports2` WHERE day = '$D' AND month = '$M'  AND year = '$Y'  AND catagory ='exp' AND casheer = '$cahseer' AND status ='done'");
while($row2 = mysql_fetch_array($result2))
{
$money4 = $row2['SUM(price)'];
}	
     //المشتروات غير المصاريف
$result3 = mysql_query("SELECT *,SUM(price) FROM `reports2` WHERE day = '$D' AND month = '$M'  AND year = '$Y'  AND catagory !='exp' AND catagory !='in' AND casheer = '$cahseer' AND status = 'done'");
...
$money3 = $row4['SUM(price)'];
     ...
     //الخصم
$result5 = mysql_query("SELECT SUM(discount2) FROM `reports` WHERE day = '$D' AND month = '$M'  AND year = '$Y' AND casheer = '$cahseer' AND status = 'done'");	
while($row5 = mysql_fetch_array($result5))
{
$money5 = $row5['SUM(discount2)'];
} 
     //الخصم 2
$result6 = mysql_query("SELECT SUM(discount_amount) FROM `reports` WHERE day = '$D' AND month = '$M'  AND year = '$Y' AND casheer = '$cahseer' AND status = 'done'");
while($row6 = mysql_fetch_array($result6))
{
$money6 = $row6['SUM(discount_amount)'];
}
     //المشتروات غير المصاريف
$result7 = mysql_query("SELECT SUM(price) FROM `reports2` WHERE day = '$D' AND month = '$M'  AND year = '$Y'   AND catagory ='in' AND casheer = '$cahseer' AND status ='done'");
while($row7 = mysql_fetch_array($result7))
{
$money7 = $row7['SUM(price)'];
}
  $query = "SELECT  SUM(discount2) FROM reports2 where day = $D AND month = $M AND year = $Y AND casheer = '$cahseer' AND status ='done'"; 
$resulty = mysql_query($query) or die(mysql_error());
// Print out result
while($row = mysql_fetch_array($resulty)){
$ds5 = $row['SUM(discount2)'];
  ...
  echo "<tr>";
   ...
   echo "<td><h1>" . $row2x['Username'] . "</h1></td>";
...
<td><a href="reports_day_casheer_all.php?se_day=<?php echo $D;?>&se_month=<?php echo $M;?>&se_year=<?php echo $Y;?>&se_cash=<?php echo $cahseer;?>"><img src="img/app/buttons/info.png"></a></td><?php 
      echo "</tr>";
}}?>
						  </tbody>
<?php 
...
<tr>
						...
						<?php 	
// الوقت
 $result = mysql_query("SELECT *,SUM(price) FROM `reports2` WHERE day = '$D' AND month = '$M'  AND year = '$Y'  AND catagory !='exp' AND catagory !='in' AND casheer !='' AND status ='done' GROUP BY casheer");					
while($row = mysql_fetch_array($result))
{
	$money1 = 0;
	$cahseer = $row['casheer'];
...
<td><h3><font color="red"><?php echo $money5+$money6+$ds5;?></font></h3></td>
<td><h2><?php echo $moneyall;?></h2></td>
<td><a href="reports_day_casheer_all.php?se_day=<?php echo $D;?>&se_month=<?php echo $M;?>&se_year=<?php echo $Y;?>&se_cash=<?php echo $row['casheer'];?>"><img src="img/app/buttons/info.png"></a></td>
<?php 
      echo "</tr>";
  }?>
						  </tbody>

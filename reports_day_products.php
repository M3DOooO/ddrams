...
<!-- left menu ends -->
			...
			<a href="reports.php"><span>التقارير</span></a> / <a href="reports_day.php?day=<?php echo $D;?>&month=<?php echo $M;?>&year=<?php echo $Y;?>"><span>تقارير يوم <?php echo $Y?>-<?php echo $M?>-<?php echo $D?></span></a> / <span>تقرير الأصناف</span>
			</div>
			<!-- content starts -->
	  	...
	  	while($roww4 = mysql_fetch_array($result4))
{
	...
	$result2 = mysql_query("SELECT *,SUM(num),SUM(price) FROM `reports2` WHERE `day` = '$D' AND month = $M AND year = $Y AND `catagory` != 'in' AND `catagory` != 'exp' AND `name` = '$thename'");
	  	while($roww = mysql_fetch_array($result2))
{
	...
	$price2 = $roww['SUM(price)'];
	  ...
	  $result3 = mysql_query("SELECT SUM(notes),SUM(price) FROM `reports2` WHERE `day` = '$D' AND month = '$M' AND year = '$Y' AND `catagory` = 'exp' AND `name` = '$thename'");

...
while($row = mysql_fetch_array($result))
{
	...
	$shift = $row['shift'];
	 // المصاريف
$result2 = mysql_query("SELECT *,SUM(price) FROM `reports2` WHERE day = '$D' AND month = '$M'  AND year = '$Y'  AND catagory ='exp' AND shift = '$shift' AND status ='done'");
...
$money2 = $row3['SUM(price)'];
     ...
     //الأوردرات
$result4 = mysql_query("SELECT ps_orders.*,SUM(price) FROM `ps_orders` INNER JOIN (SELECT DISTINCT session_id FROM reports WHERE day = '$D' AND month = '$M' AND year = '$Y' AND shift = '$shift' AND status = 'done') AS shift_sessions ON ps_orders.session_id = shift_sessions.session_id WHERE ps_orders.shift = '$shift' AND ps_orders.status ='yes'");
while($row4 = mysql_fetch_array($result4))
{
$money3 = $row4['SUM(price)'];
}
$result33 = mysql_query("SELECT SUM(discount2) FROM `reports2` WHERE day = '$D' AND month = '$M'  AND year = '$Y' AND shift = '$shift' AND status = 'done'");
...
$money6 = $row6['SUM(discount_amount)'];
     ...
     //المشتروات غير المصاريف
$result7 = mysql_query("SELECT SUM(price) FROM `reports2` WHERE day = '$D' AND month = '$M'  AND year = '$Y'   AND catagory ='in' AND shift = '$shift' AND status = 'done'");
